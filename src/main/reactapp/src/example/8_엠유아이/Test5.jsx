...
export default function MemberInfo() {
  ...
  const [message, setMessage] = React.useState(''); // 탈퇴 성공/실패 메시지
  ...
  const handleDelete = async () => {
    if (window.confirm('정말로 회원 탈퇴를 하시겠습니까?')) {
      setLoading(true);
      try {
        const response = await fetch(`/member/info?mno=${mno}`, { method: 'DELETE' });
        const data = await response.json();
        if (response.ok) {
          setMessage('회원 탈퇴가 성공적으로 처리되었습니다.');
          setMemberData(null); // 탈퇴 후 회원 정보 초기화
        } else {
    ...
    <ThemeProvider theme={theme}>
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', padding: '20px' }}>
        <Card variant="outlined" sx={{ width: '100%', maxWidth: '850px', padding: '50px', marginTop: '50px', borderRadius: '12px', position: 'relative' }}>
          ...
          {message && <Typography sx={{ textAlign: 'center', color: 'green' }}>{message}</Typography>}

          {memberData && (
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <Typography variant="h6">회원 번호: {memberData.mno}</Typography>
              ...
              <Box sx={{ marginTop: '20px' }}>
                <Button
                  ...
                  disabled={loading}
                >
                  {loading ? <CircularProgress size={24} sx={{ color: 'white' }} /> : '회원 탈퇴'}
                </Button>
              </Box>
            </Box>
          )}
